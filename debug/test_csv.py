...
def split_dataset(csv_path, train=0.7, val=0.15):
    dataset = pd.read_csv(csv_path)
    train_samples = dataset.groupby("map", group_keys=False).apply(lambda x:x.sample(frac=train)).reset_index(drop=True)
    test_samples = dataset.iloc[dataset.index.difference(train_samples.index)]

    val_samples = train_samples.groupby("map", group_keys=False).apply(lambda x:x.sample(frac=(train*val)))

    train_samples = train_samples.iloc[train_samples.index.difference(val_samples.index)]
    ...
    print(len(train_samples), train_samples)
