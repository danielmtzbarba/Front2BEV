...
class Front2BEVDataset(Dataset):
    ...
    def __init__(self, df, image_size, output_size, num_class, rgbd=False):
        ...
        self.transform = Transforms([Rescale(self.image_size), Normalize()])
    ...
    def load_class_masks(self, idx):
        ...
        labels, mask = (
            decoded[:-1],
            torch.from_numpy(np.ones_like(bev.resize_img(mask64, self.output_size))),
        )
        return labels, mask
