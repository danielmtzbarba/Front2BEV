...
def dilated_class(sem_img, bev_img, cmap, size, k=3, i=3, morph=True):
    bev_img = bev_img.copy()
    cls_mask = sem_img.copy()
    cls_mask = (sem_img == cmap[0]).astype(np.uint8)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (k, k))
    cls_mask = cv2.resize(cv2.dilate(cls_mask, kernel, iterations=i), size)
    if morph:
...
def postprocess(sem_img, bev_map, size, fov_mask, n_classes, morph=True, display=False):
    ...
    if morph:
        ...
        if n_classes > 5:
            bev_img = dilated_class(
                sem_img, eroded, [190, 5], size, k=5, i=3, morph=True
            )
        if n_classes > 4:
            bev_img = dilated_class(
                sem_img, eroded, [84, 4], size, k=3, i=3, morph=True
            )
        else:
            bev_img = eroded
