...
decoded_masks = bev.postprocess(bev_sem, bev_cls[N_CLASSES], size, 
                        bev.resize_img(mask64, size), N_CLASSES, morph=True)

#encoded_mask = encode_binary_labels(decoded_masks)
#Image.fromarray(encoded_mask.astype(np.int32), mode='I').save('test.png')
#print('Image saved')

img = np.array(Image.open('/media/danielmtz/data/datasets/Dan-2023-Front2BEV/Town06/layers_all/bev/2k/0.png'))
encoded = to_tensor(img).long()
decoded = decode_binary_labels(encoded, N_CLASSES + 1).numpy().reshape(*size, N_CLASSES + 1)
